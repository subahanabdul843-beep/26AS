...
F_STATUS  = r'([A-Z])'
F_SECTION = r'([0-9]{1,4}[A-Z0-9/\-]{0,10}(?:\([A-Za-z0-9]+\))*)'
F_AMOUNT  = r'(-?[\d,]+\.\d{2}|-?[\d,]+)'
F_REMARK  = r'(\S+)'
F_NAME    = r'([A-Z0-9 &.,()\'/-]+?)'
    ...
    for line in part_text.split("\n"):
        ...
        if sm:
            ...
            summary_rows.append({
                ...
                f"Total Tax {'Collected' if role == 'Collector' else 'Deducted'}": safe_float(sm.group(5)),
                f"Total {'TCS' if role == 'Collector' else 'TDS'} Deposited": safe_float(sm.group(6)),
            })
            continue
...
def amount_column_for(df: pd.DataFrame) -> str:
    """Pick the primary 'amount' column to total, based on naming
    convention rather than a hardcoded per-Part list."""
    if df is None or df.empty:
        return None
    preferred_keywords = ["Amount Paid", "Amount Paid/Debited", "Amount Paid/Credited",
                           "Refund Amount", "Total Transaction Amount", "Total Amount"]
...
def tax_column_for(df: pd.DataFrame) -> str:
    """Pick the 'Tax Deducted'/'Tax Collected' column for a DataFrame,
    based on naming convention."""
    if df is None or df.empty:
        return None
    preferred_keywords = ["Total Tax Deducted", "Total Tax Collected", "Tax Deducted", "Tax Collected"]
    for kw in preferred_keywords:
        for c in df.columns:
            if kw.lower() in c.lower() and pd.api.types.is_numeric_dtype(df[c]):
                return c
...
def deposited_column_for(df: pd.DataFrame) -> str:
    """Pick the 'TDS Deposited'/'TCS Deposited' column for a DataFrame,
    based on naming convention."""
    if df is None or df.empty:
        return None
    preferred_keywords = ["Total TDS Deposited", "Total TCS Deposited", "TDS Deposited", "TCS Deposited"]
    for kw in preferred_keywords:
        for c in df.columns:
            if kw.lower() in c.lower() and pd.api.types.is_numeric_dtype(df[c]):
                return c
...
if uploaded_file:
    ...
    # Totals are derived from each Part's own printed Summary table
    # (Name/TAN + Total Amount/Tax/Deposited rows). Parts with no data
    # simply contribute 0 here — they are NOT skipped from this sheet.
    overview_rows = []
    for pr in part_results:
        sdf = pr["summary_df"]
        n_records = len(sdf) if sdf is not None else 0
 
        amt_col = amount_column_for(sdf)
        tax_col = tax_column_for(sdf)
        dep_col = deposited_column_for(sdf)
 
        total_amount = safe_float(sdf[amt_col].sum()) if amt_col else 0.0
        total_tax = safe_float(sdf[tax_col].sum()) if tax_col else 0.0
        total_deposited = safe_float(sdf[dep_col].sum()) if dep_col else 0.0
    ...
    overview_df = pd.DataFrame(overview_rows, columns=[
        "Part", "Description", "Contact", "Number of Records",
        "Total Amount", "Tax Deducted/Collected", "TDS/TCS Deposited",
    ])
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        total_pairs = [(5, get_column_letter(5)), (6, get_column_letter(6)), (7, get_column_letter(7))]
        apply_sheet_formatting(
            ws,
            numeric_cols=[5, 6, 7],
        ...
        # ---- Sheet 2: Combined Part Summary (all Parts in one sheet) ---
        # Every Part's deductor/collector summary rows are stacked into a
        # single sheet, in document order, with a leading "Part" column.
        # Parts with no data simply contribute no rows here (the sheet is
        # still created, since at least one Part normally has data; the
        # "No Data" requirement is satisfied by the overview sheet above).
        ...
        for pr in part_results:
            ...
            combined_summary_frames.append(labeled)
 
        if combined_summary_frames:
            combined_summary_df = pd.concat(combined_summary_frames, ignore_index=True, sort=False)
        else:
            combined_summary_df = pd.DataFrame()
 
        num_cols_summary = [c for c in (combined_summary_df.columns if not combined_summary_df.empty else [])
                             if pd.api.types.is_numeric_dtype(combined_summary_df[c]) and c != "S.No"]
        write_df_sheet(
            writer, combined_summary_df, "Part Summary",
            numeric_col_names=num_cols_summary,
            add_totals=bool(num_cols_summary),
            table_prefix="PartSummary",
